...
def svm_loss(x, y):
    ...
    margin = np.maximum(x - correct_class_scores + 1, 0)
    loss = np.sum(margin, axis=-1).mean() - 1
...
def softmax_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = x.shape[0]
    exp_scores = np.exp(x - x.max(axis=-1).reshape(-1, 1))
    sum_exp_scores = exp_scores.sum(axis=-1)
    correct_class_scores = exp_scores[np.arange(N), y]
    loss = -np.log(correct_class_scores / sum_exp_scores).mean()
