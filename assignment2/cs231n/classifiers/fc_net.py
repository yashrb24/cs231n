...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for l, (i, j) in enumerate(zip(in_dims, out_dims)):
            self.params[f'W{l+1}'] = np.random.normal(loc=0, scale=weight_scale, size=(i, j))
    ...
    def loss(self, X, y=None):
        ...
        caches = []
        out = X
        bn_params = [{"mode": "train"} for _ in range(self.num_layers)]
        for i in range(self.num_layers):
            curr_cache = []
           
            a, fc_cache = affine_forward(out, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
            curr_cache.append(fc_cache)

            if i < self.num_layers - 1:
                if self.normalization is not None:
                    a, bn_cache = batchnorm_forward(a, self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], bn_params[i])
                    curr_cache.append(bn_cache)
                out, relu_cache = relu_forward(a)
                curr_cache.append(relu_cache)
                if self.use_dropout:
        ...
        for i in reversed(range(self.num_layers)):
            if i < self.num_layers - 1:
                if self.normalization is not None:
                    ...
                    if self.use_dropout:
                        fc_cache, relu_cache, dropout_cache = caches[i]
                        dout = dropout_backward(dout, dropout_cache)
                    else:
            ...
            grads[f"W{i+1}"] = dw + self.reg * fc_cache[1]
            grads[f"b{i+1}"] = db
            loss += 0.5 * self.reg * np.sum(fc_cache[1] * fc_cache[1]) 
