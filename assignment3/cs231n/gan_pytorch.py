...
def discriminator_loss(logits_real, logits_fake):
    ...
    target_real = torch.ones((N, )).type(dtype)
    target_fake = torch.zeros((N, )).type(dtype)
...
def generator_loss(logits_fake):
    ...
    N = logits_fake.shape[0]
    target = torch.ones((N, )).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = 0.5 * torch.square(scores_real - 1).mean()
    loss_fake = 0.5 * torch.square(scores_fake).mean()
    loss = loss_real + loss_fake
