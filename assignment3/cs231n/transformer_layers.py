...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # (N, S, H, E // H) @ (N, H, E // H, T) -> (N, S, H, T)
        Y = torch.matmul(Q, K.moveaxis(2, 3)) / scale
        
        # apply mask
        if attn_mask is not None:
            Y = Y.masked_fill(attn_mask == 0, float('-inf'))

        # (N, S, H, T) @ (N, H, T, E // H) -> (N, S, H, E // H)
        attention_scores = self.attn_drop(F.softmax(Y, dim=-1)) @ V
        
        # concat all heads and project
        # (N, S, H, E // H) -> (N, S, E)
        output = self.proj(attention_scores.moveaxis(1, 2).reshape(N, S, E))
