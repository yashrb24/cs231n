...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        embeddings, cache1 = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == "rnn":
            h, cache2 = rnn_forward(embeddings, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        current_embed, _ = word_embedding_forward(current_word, W_embed)

        for t in range(1, max_length):
            current_h, _ = rnn_step_forward(current_embed, current_h, Wx, Wh, b)
            scores, _ = affine_forward(current_h, W_vocab, b_vocab)
            word_indices = np.argmax(scores, axis=-1)
            captions[:, t] = word_indices
            current_embed, _ = word_embedding_forward(word_indices, W_embed)
