...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(z)
    cache = [next_h, prev_h, x, Wx, Wh, b, z]
...
def rnn_step_backward(dnext_h, cache):
    ...
    next_h, prev_h, x, Wx, Wh, b, z = cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        curr_x = x[:, t, :]
        next_h, curr_cache = rnn_step_forward(curr_x, next_h, Wx, Wh, b)
        h[t] = next_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        # Combine upstream gradient from loss and recurrent connection
        dcurrent_h = dh[:, t, :] + dprev_h
        dx_curr, dprev_h, dWx_curr, dWh_curr, db_curr = rnn_step_backward(dcurrent_h, cache[t])
